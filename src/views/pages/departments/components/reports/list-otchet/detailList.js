...
export default function DetailList({ datas }) {
    const { id } = useParams();

    const selectedData = typeof datas === 'object' ? datas.find((elem) => elem._id === id) : null;

    return (
        <CCard>
            {selectedData ? (
                <div>
                    <h1 className='text-center mt-10 mb-10'>Дата: {selectedData.date}</h1>
                    <PercentUI />
