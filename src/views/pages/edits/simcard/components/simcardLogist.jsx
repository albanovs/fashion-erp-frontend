...
export default function SimcardLogist({ datas, api, addSlot, slot, update, title }) {
    ...
    const curatorChange = async (id) => {
        try {
            ...
            await fetchApi.patch(`/test/logist-curator${api}/${id}`, datas);
            window.location.reload()
        } catch (error) {
            console.log(error);
        }
    ...
    return (
        <>
            <div className="mb-4">
                <table className="table w-full rounded-sm mb-4">
                    <thead>
                        <tr>
            ...
            </div>
            {
                loading ? (datas.map((elem, index) => (
                    <div key={elem._id} className="mt-10">
                        ...
                        <table className='w-full bg-white text-center border border-collapse'>
                            <thead className='bg-gradient-to-r lg:text-[12px] text-[4px] from-blue-500 to-purple-500 text-white'>
                                <tr>
                                    ...
                                    <th className='lg:py-1 lg:px-2 border'>Старший админ</th>
                                ...
                                <tr>
                                    ...
                                    <th className='lg:py-1 lg:px-2 border'>помощник админа</th>
                                        ...
                                        <tbody key={item._id} className={isOdd ? "bg-gray-100 text-[4px] lg:text-[10px]" : "text-[4px] lg:text-[10px]"}>
                                            <tr>
                                                <th className='border lg:py-1 lg:px-3' onClick={async () => {
                                                    try {
                                                        await fetchApi.delete(`/test/logist-deleteSlot${api}/${item._id}`);
                                                        window.location.reload()
                                                    } catch (error) {
                        ...
                        <button className='lg:ml-3 mt-5 mb-5 bg-gradient-to-r from-blue-500 to-purple-500 hover:bg-blue-600 text-white lg:py-2 lg:px-6 p-1 rounded-lg shadow-lg transition-all duration-300 ease-in-out text-[10px] transform hover:scale-105 focus:outline-none' onClick={() => addSlots(elem._id)}>добавить слот</button>
                    </div>
                ))) : <div>Загрузка...</div>
            }
            {
